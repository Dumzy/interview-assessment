package com.zinkwork.Atm.constant;

public final class AtmValidationMessages {

    public static final String ACCOUNT_NO_REQUIRED = "Account Number is Mandatory";
    public static final String PIN_NO_REQUIRED = "PIN Number is Mandatory";
    public static final String INVALID_WITHDRAWAL_AMOUNT = "Invalid Withdrawal Amount";
    public static final String INVALID_ACCOUNT = "Please Check Account Number & Pin";
    public static final String NO_ENOUGH_BALANCE = "You don't have enough account balance to withdraw money";
    public static final String NO_ENOUGH_BANK_BALANCE = "Bank doesn't have enough money to withdraw this Amount";
    public static final String NO_ENOUGH_BANK_NOTES = "Bank doesn't have enough notes to withdraw this Amount";
    public static final String INVALID_WITHDRAWAL_VALUE = "Withdrawal Amount Should be in Valid Currency Notes";

    public static final String INVALID_ATM_AMOUNT = "Invalid Amount";
    public static final String INVALID_FIFTY_NOTES = "Invalid Number of Fifty Notes";
    public static final String INVALID_TWENTY_NOTES = "Invalid Number of Twenty Notes";
    public static final String INVALID_TEN_NOTES = "Invalid Number of Ten Notes";
    public static final String INVALID_FIVE_NOTES = "Invalid Number of Five Notes";

}
